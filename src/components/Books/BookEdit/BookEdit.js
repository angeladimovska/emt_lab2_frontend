...
const BookEdit = (props) => {
    ...
    const onFormSubmit = (e) => {
        ...
        const category = formData.category !== 0 ? formData.category : props.book.category.name;
                        ...
                        <select name="category" className="form-control" onChange={handleChange}>
                            {props.categories.map((term) => {
                                if (props.book.category !== undefined &&
                                    props.book.category.name === term.name)
                                    return <option selected={props.book.category.name}
                                                   value={term.name}>{term.name}</option>
                                else return <option value={term.name}>{term.name}</option>
                            })}
                        </select>
                    </div>
                    <div className="form-group">
